...
i2c_sensor_in_hook(
		...
		void * param)
{
	i2c_sensor_t * p = (i2c_sensor_t*)param;
	avr_twi_msg_irq_t v;
	v.u.v = value;
	...
	if (p->selected) {
		...
		if (v.u.twi.msg & TWI_COND_WRITE) {
			...
			if (p->index < addr_size) {
				...
				if (p->index == addr_size-1) {
					...
					if (p->verbose)
						printf("sensor set address to 0x%04x\n", p->reg_addr);
				}
			} else {
				if (p->verbose)
					printf("sensor WRITE data 0x%04x: %02x\n", p->reg_addr, v.u.twi.data);
				p->input[p->reg_addr++] = v.u.twi.data;
			}
			p->reg_addr &= (p->size -1);
			p->index++;
		}
		...
		if (v.u.twi.msg & TWI_COND_READ) {
			if (p->verbose)
				printf("sensor READ data 0x%04x: %02x\n", p->reg_addr, p->output[p->reg_addr]);
			uint8_t data = p->output[p->reg_addr++];
			avr_raise_irq(p->irq + TWI_IRQ_INPUT,
...
static const char * _ee_irq_names[2] = {
		[TWI_IRQ_INPUT] = "8>sensor.out",
		[TWI_IRQ_OUTPUT] = "32<sensor.in",
};
...
{
	memset(p, 0, sizeof(*p));
	memset(p->input, 0xff, sizeof(p->input));
	memset(p->output, 0xff, sizeof(p->output));

	p->addr_base = addr;
	p->addr_mask = mask;

	p->irq = avr_alloc_irq(&avr->irq_pool, 0, 2, _ee_irq_names);
	avr_irq_register_notify(p->irq + TWI_IRQ_OUTPUT, i2c_sensor_in_hook, p);

	p->size = size > sizeof(p->input) ? sizeof(p->input) : size;
	if (data)
		memcpy(p->input, data, p->size);
}
